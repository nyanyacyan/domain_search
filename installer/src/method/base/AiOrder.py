...
from .fileWrite import LimitFileWrite
...
# SNSバージョン　リクエストした文章とresponseで帰ってきた文章を整理してjsonファイルに書き込む
    ...
    async def resultSave(
        ...
        userToMsg = self.userToNotifyMsg(userMsg=userMsg, assistantMsg=assistantMsg, notifyMsg=notifyMsg)
...
# ----------------------------------------------------------------------------------
    ...
    async def reRequest(self, fixedPrompt :str, beforeResult: dict, endpointUrl: str, model: str, apiKey: str, maxTokens: int):
        ...
        assistantMsg = await self.assistantResponseMessage(endpointUrl=endpointUrl, model=model, apiKey=apiKey, messages=chatHistory, maxTokens=maxTokens)
...
# ----------------------------------------------------------------------------------
    ...
    async def ChatHistory(self, prompt: str, endpointUrl: str, model: str, apiKey: str, maxTokens: int):
        chatHistory = []
